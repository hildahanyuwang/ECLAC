...
# Filter rows for 15-19 years and convert values to numeric
df = df[df["Dim2"] == "15-19 years"]
df["First Tooltip"] = pd.to_numeric(df["First Tooltip"], errors="coerce")
df = df.dropna(subset=["First Tooltip"])  # remove rows where value is NaN
...
ax.axhline(y=10, color="#9dc1c5", linestyle="--", linewidth=1, zorder=2)
ax.axhline(y=60, color="#d7a6b3", linestyle="--", linewidth=1, zorder=2)

ax.set_title("Regional Adolescent Birth Rates(15-19 years)", fontsize=12, fontweight="bold")
